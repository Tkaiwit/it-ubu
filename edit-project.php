...
<!-- <div id="icon-menu">
		<div>
			...
			<div id="detailone">
				<div id="toppic-detail"><label>Edit Project</label>
				</div>
				<form action="connectDB/edit/editproject.php" method="POST" id="form-slide-img" enctype="multipart/form-data" Multiple>
			<div id="add-slide-img">
				<?php 
				include('connectDB/connectDB.php');
				$obj="SELECT * FROM project where member_id";
				$ses=mysqli_query($conDB,$obj);
				while ($roww=mysqli_fetch_array($ses)) {
					if($_SESSION['member_id'] == $roww['member_id']){
				?>
				<div>
					<input type="text" name="p_name" id="slide-topic" placeholder="หัวข้อเรื่อง" value="<?php echo $roww['project_name']?>"><br><br>
					<input type="text" value="<?php echo $roww['project_year']?>" name="p_year" id="slide-name-button" placeholder="ปีการศึกษา">
				</div>
				<div>
					<textarea name="p_contant" cols="30" rows="5" id="slide-decription" placeholder="รายละเอียด"><?php echo $roww['project_contant']?></textarea>
				</div>
				<div>
					<select name="c_id" id="teacher-drop">
						...
						?>
						<option <?php if($roww['categries_id'] == $rows['categries_id']){echo 'selected';}?> value="<?php echo $rows['categries_id']?>"><?php echo $rows['categries_name']?></option>
					<?php
						}
					?>
					</select>
				...
				<div>
					<select name="t_id" id="teacher-drop">
						...
						<option <?php if($roww['teacher_id'] == $row['teacher_id']){echo 'selected';} ?> name="t_id" value="<?php echo $roww['teacher_id']?>">
				...
				<div id="img-demo-container"> 
					<!-- <input type="file" name="slide-img-name"  onchange="readURL(this);" placeholder=""> -->
  					...
  					<input type="file" name="image" value="<?php echo $roww['project_image']?>" id="file-upload" multiple/>
				</div>
				...
				<div id="video-demo-container">
					...
					<input type="file" name="file" id="file-to-upload" accept="video/mp4" value="<?php echo $roww['project_video_location']?>" />
				</div>
				</fieldset>
				<fieldset>
				...
				<div class="custom-file-upload" style="margin-left: 70px;">
					...
					<input type="file" name="myfile" id="file-zip-upload" accept=".rar"  value="<?php echo $roww['project_zip']?>" />
				</div>
				</fieldset>
				<fieldset>
